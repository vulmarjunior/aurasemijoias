    ...
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
        <div className="p-6 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <h3 className="text-lg font-bold text-slate-900">Vendas</h3>
          {canEdit(user?.perfil) && (
        ...
        <div className="p-4 border-b border-slate-100">
          <div className="relative w-full sm:max-w-xs">
            ...
            <input value={searchVenda} onChange={e => setSearchVenda(e.target.value)} placeholder="Buscar por cliente ou pagamento..." className="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-slate-400 m-6 border border-dashed rounded-lg border-slate-300">Nenhuma venda encontrada.</div>
        ) : (
          <div>
            {filteredVendas.map(v => {
              ...
              return (
                <div key={v.id} className="border-b border-slate-50 last:border-b-0">
                  <div className="flex items-center justify-between px-4 py-3 hover:bg-slate-50/50 cursor-pointer transition-colors" onClick={() => { setExpandedId(expandedId === v.id ? null : v.id); if (expandedId !== v.id) fetchItens(v.id) }}>
                    <div className="flex items-center gap-4 flex-1">
                      <div className="text-sm text-slate-500 font-mono w-24">{new Date(v.data_venda).toLocaleDateString('pt-BR')}</div>
                      <div className="text-sm font-medium text-slate-900 flex-1">{v.clientes?.nome || 'Cliente removido'}</div>
                      <div className="text-xs bg-slate-100 px-2 py-0.5 rounded-full text-slate-600">{v.forma_pagamento}</div>
                      <div className="text-sm font-bold text-green-700">{Number(v.valor_total).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</div>
                    </div>
                    {expandedId === v.id ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
                  </div>
                  {expandedId === v.id && (
                    <div className="px-4 pb-3 bg-slate-50/50">
                      {itensVenda[v.id] ? (
                        <>
                            ...
                            <tbody>
                              {itensVenda[v.id].map(item => (
                                <tr key={item.id} className="border-t border-slate-100">
                                  <td className="px-3 py-2 text-slate-900">{item.produtos?.nome || 'Produto removido'}</td>
                          ...
                          {desconto > 0 && (
                            <div className="text-xs text-right mt-2 text-slate-500 pr-3">
                              Desconto: <span className="text-red-500 font-medium">-{desconto.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                            </div>
                          )}
                          {texto && <div className="text-xs text-slate-500 mt-2 px-3 italic">Obs: {texto}</div>}
                        </>
                      ) : (
                        <div className="text-center py-3 text-slate-400 text-sm">Carregando itens...</div>
                      )}
                    </div>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h4 className="text-lg font-bold text-slate-900">Nova Venda</h4>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">Data da Venda</label>
                  <input type="date" value={dataVenda} onChange={e => setDataVenda(e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">Forma de Pagamento</label>
                  <select value={formaPagamento} onChange={e => setFormaPagamento(e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <select value={clienteId} onChange={e => setClienteId(e.target.value)} className="flex-1 px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white">
                    <option value="">Selecione um cliente...</option>
                    {clientes.map(c => <option key={c.id} value={c.id}>{c.nome}</option>)}
                  </select>
                  <button type="button" onClick={() => setClientModalOpen(true)} className="px-3 py-2 border border-slate-200 rounded-lg text-slate-500 hover:text-blue-600 hover:border-blue-200 transition-colors" title="Cadastrar novo cliente">
              ...
              <div className="border-t border-slate-100 pt-4">
                <label className="block text-xs font-semibold text-slate-500 mb-3">Produtos</label>
                <div className="flex gap-2 items-end">
                  <div className="flex-1 relative">
                    <label className="block text-xs text-slate-400 mb-1">Produto</label>
                    {selectedProdId ? (
                      <div className="flex items-center gap-2 px-3 py-2 text-sm border border-blue-200 bg-blue-50 rounded-lg">
                        <span className="flex-1 truncate">{selectedProd.codigo_peca || ''} {selectedProd.nome} <span className="text-slate-400 text-xs">(estoque: {selectedProd.quantidade})</span></span>
                        <button onClick={() => { setSelectedProdId(''); setSearchProd('') }} className="text-slate-400 hover:text-slate-600"><X className="w-4 h-4" /></button>
                      </div>
                    ) : (
                      <>
                        <input value={searchProd} onChange={e => { setSearchProd(e.target.value); setShowProdResults(true); setHighlightIdx(-1) }} onFocus={() => setShowProdResults(true)} onKeyDown={e => { if (e.key === 'ArrowDown') { e.preventDefault(); setHighlightIdx(i => Math.min(i + 1, prodResults.length - 1)) } else if (e.key === 'ArrowUp') { e.preventDefault(); setHighlightIdx(i => Math.max(i - 1, 0)) } else if (e.key === 'Enter' && highlightIdx >= 0 && prodResults[highlightIdx]) { setSelectedProdId(prodResults[highlightIdx].id); setSelectedQty(1); setShowProdResults(false); setSearchProd('') } else if (e.key === 'Escape') setShowProdResults(false) }} placeholder="Buscar por nome, código ou referência..." className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                        {showProdResults && searchProd.trim() && (
                          <div className="absolute z-10 mt-1 w-full bg-white border border-slate-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                            {prodResults.length === 0 ? (
                              ...
                              prodResults.map((p, i) => (
                                <button key={p.id} onMouseEnter={() => setHighlightIdx(i)} onClick={() => { setSelectedProdId(p.id); setSelectedQty(1); setShowProdResults(false); setSearchProd('') }} className={`w-full text-left px-3 py-2 text-sm flex items-center gap-2 hover:bg-blue-50 transition-colors ${i === highlightIdx ? 'bg-blue-50' : ''}`}>
                                  <span className="font-medium text-slate-900">{p.codigo_peca || '---'}</span>
                                  <span className="text-slate-600 truncate">{p.nome}</span>
                                  <span className="ml-auto text-xs text-slate-400 shrink-0">estoque: {p.quantidade}</span>
                                </button>
                              ))
                            )}
                          </div>
                  ...
                  <div className="w-24">
                    ...
                    <input type="number" min="1" max={selectedProd?.quantidade || 1} value={selectedQty} onChange={e => setSelectedQty(Number(e.target.value))} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                  </div>
                  <button onClick={addToCart} disabled={!selectedProdId} className="px-3 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white rounded-lg text-sm font-semibold transition-colors">Adicionar</button>
                </div>
                ...
                {cart.length > 0 && (
                  <div className="mt-3 bg-slate-50 rounded-lg p-3 space-y-3">
                      ...
                      <tbody>
                        {cart.map((item, idx) => (
                          <tr key={idx} className="border-t border-slate-200">
                            <td className="py-1.5 pr-2 text-slate-900">{item.nome}</td>
                    ...
                    <div className="flex items-center justify-end gap-3 pt-2 border-t border-slate-200">
                      ...
                      <div className="text-sm font-semibold text-slate-900 w-28 text-right">{subtotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</div>
                    </div>
                    <div className="flex items-center justify-end gap-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => { setDescontoTipo('percentual'); setDescontoInput('') }} className={`px-2 py-1 text-xs rounded transition-colors ${descontoTipo === 'percentual' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}><Percent className="w-3 h-3" /></button>
                        <button onClick={() => { setDescontoTipo('fixo'); setDescontoInput('') }} className={`px-2 py-1 text-xs rounded transition-colors ${descontoTipo === 'fixo' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}><DollarSign className="w-3 h-3" /></button>
                        <input type="number" min="0" step={descontoTipo === 'percentual' ? '1' : '0.01'} value={descontoInput} onChange={e => setDescontoInput(e.target.value)} placeholder={descontoTipo === 'percentual' ? '%' : 'R$'} className="w-20 px-2 py-1 text-sm text-right border border-slate-200 rounded focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                      </div>
                      {descontoFinal > 0 && (
                    ...
                    <div className="flex items-center justify-end gap-3 border-t-2 border-slate-300 pt-2 font-bold">
              ...
              <div>
                ...
                <textarea value={observacoes} onChange={e => setObservacoes(e.target.value)} rows={2} placeholder="Opcional..." className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 resize-none" />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
              ...
              <button onClick={handleSave} disabled={saving || !clienteId || cart.length === 0} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white rounded-lg text-sm font-semibold transition-colors flex items-center gap-1.5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setClientModalOpen(false)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setClientModalOpen(false)} className="text-slate-400 hover:text-slate-600"><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1">Nome *</label>
                <input value={newClientNome} onChange={e => setNewClientNome(e.target.value)} placeholder="Nome do cliente" autoFocus className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1">Telefone</label>
                <input value={newClientTelefone} onChange={e => setNewClientTelefone(e.target.value)} placeholder="(11) 99999-9999" className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1">Instagram</label>
                <input value={newClientInstagram} onChange={e => setNewClientInstagram(e.target.value)} placeholder="@cliente" className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
              </div>
            </div>
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
              <button onClick={() => setClientModalOpen(false)} className="px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors">Cancelar</button>
              <button onClick={handleQuickClient} disabled={savingClient || !newClientNome.trim()} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white rounded-lg text-sm font-semibold transition-colors flex items-center gap-1.5">
