    ...
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
        <div className="p-6 border-b border-slate-200 flex justify-between items-center">
          <h3 className="text-lg font-bold text-slate-900">Categorias de Semijoias</h3>
          {canEdit(user?.perfil) && (
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-slate-400 m-6 border border-dashed rounded-lg border-slate-300">Nenhuma categoria cadastrada.</div>
        ) : (
                ...
                {categorias.map(c => (
                  <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="text-sm text-slate-600">{totalProdutos[c.id] || 0}</span>
                    </td>
                    <td className="px-4 py-3 text-center">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <input value={nome} onChange={e => setNome(e.target.value)} placeholder="Ex: PINGENTE" className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 uppercase" />
            </div>
